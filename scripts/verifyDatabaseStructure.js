...
const expectedSubcollections = {
  professionals: ['services', 'workingHours', 'timeOff', 'portfolio', 'licenses'],
...
async function verifySubcollections(collectionId) {
  ...
  console.log(
    `Found subcollections for ${collectionId}/${docId}:`,
  ...
  if (missingSubcollections.length > 0) {
    console.warn(
      `Missing expected subcollections for ${collectionId}:`,
      missingSubcollections.join(', ')
    );
...
async function verifyUserCounts() {
  ...
  if (totalRoleSpecific < userCount) {
    console.warn(
      `Warning: There are ${userCount} users but only ${totalRoleSpecific} in role-specific collections.`
    );
    console.warn('Some users may not have been migrated properly.');
  } else if (totalRoleSpecific > userCount) {
    console.warn(
      `Warning: There are ${totalRoleSpecific} users in role-specific collections but only ${userCount} in the users collection.`
    );
