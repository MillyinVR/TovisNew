...
async function migrateToNewAuthStructure() {
  ...
  try {
    ...
    const pendingProfessionals = await db.collection('users')
...
if (require.main === module) {
  ...
  if (command === 'verify') {
    verifyMigration().then(() => process.exit(0)).catch(() => process.exit(1));
  } else {
    migrateToNewAuthStructure().then(() => process.exit(0)).catch(() => process.exit(1));
  }
}
