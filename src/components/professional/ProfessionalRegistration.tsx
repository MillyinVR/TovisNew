...
export const ProfessionalRegistration = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const uploadPromises: Promise<string | null>[] = [uploadDocument(idFile, 'government_id')];

      if (professionalType !== 'makeup_artist' && licenseFile) {
        uploadPromises.push(uploadDocument(licenseFile, 'license'));
      } else {
        uploadPromises.push(Promise.resolve(null));
      }

      if (professionalType === 'makeup_artist') {
        if (verificationFileOne) {
          uploadPromises.push(uploadDocument(verificationFileOne, 'verification_one'));
        } else {
          uploadPromises.push(Promise.resolve(null));
        }
        if (verificationFileTwo) {
          uploadPromises.push(uploadDocument(verificationFileTwo, 'verification_two'));
        } else {
          uploadPromises.push(Promise.resolve(null));
        }
      } else {
      ...
      const verificationDoc = {
        ...
        professionalType,
        licenseState: professionalType !== 'makeup_artist' ? licenseState : undefined,
        licenseNumber: professionalType !== 'makeup_artist' ? licenseNumber : undefined,
        licenseExpirationDate: professionalType !== 'makeup_artist' ? licenseExpiration : undefined,
        licenseImageUrl: licenseUrl,
        verificationTypeOne: professionalType === 'makeup_artist' ? verificationTypeOne : undefined,
        verificationTypeTwo: professionalType === 'makeup_artist' ? verificationTypeTwo : undefined,
        verificationImageOne: verificationOneUrl,
      ...
      if (currentUser?.uid) {
        await setDoc(doc(verificationRef, currentUser.uid), verificationDoc);
