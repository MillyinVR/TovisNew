...
import { getProfessionalAppointments, updateAppointmentStatus } from '../lib/api/appointments';
...
export const useProfessionalAppointments = () => {
          ...
          prev.map((appt) =>
            appt.id === appointmentId
              ? {
                  ...
                  statusReason: 'Approved by professional',
                }
              : appt
          )
        );
  ...
  const cancelAppointment = useCallback(
    async (appointmentId: string, reason?: string) => {
      ...
      try {
        ...
        setCalendarEvents((prev) =>
          prev.map((event) =>
            event.id === appointmentId ? { ...event, status: AppointmentStatus.CANCELLED } : event
          )
        );
  ...
  const rescheduleAppointment = useCallback(
    async (appointmentId: string, startTime: string, endTime: string) => {
      if (!currentUser?.uid) return;
