    ...
    async (clientId: string): Promise<boolean> => {
      ...
      try {
        ...
        if (userProfile?.role === 'professional') {
          ...
          console.log(
            'Professional has appointments with client:',
            hasAppointments,
            'Count:',
          ...
          if (!hasAppointments) {
            ...
            console.log(
              'Professional has any appointments with client:',
              hasAnyAppointments,
              'Count:',
              allAppointmentsSnapshot.size
            );
  ...
  const fetchClientProfile = useCallback(
    async (clientId: string): Promise<void> => {
      ...
      try {
        ...
        if (userProfile?.role === 'admin') {
          notesQuery = query(notesRef, where('clientId', '==', clientId));
        } else {
        ...
        const aftercareQuery = query(aftercareRef, where('clientId', '==', clientId));
