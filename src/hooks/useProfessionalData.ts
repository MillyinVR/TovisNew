  ...
  const fetchProfessionalData = useCallback(async () => {
    ...
    try {
      ...
      if (!professionalDoc.exists()) {
        ...
        if (includeWorkingHours) {
          const workingHoursRef = collection(
            db,
            'professionals',
            targetProfessionalId,
            'workingHours'
  ...
  const fetchNearbyProfessionals = useCallback(async () => {
    ...
    try {
      ...
      professionalsData = professionalsData
        .filter(
          (prof) =>
            prof.distance !== null && prof.distance !== undefined && prof.distance <= radiusKm
        )
...
export const useTrendingContent = (
  ...
  const fetchTrendingContent = useCallback(async () => {
    ...
    try {
      ...
      let trendingQuery = query(collection(db, 'theLooks'), orderBy('trendingScore', 'desc'));

      // Add category filter if provided
      if (categoryId) {
        trendingQuery = query(trendingQuery, where('categoryId', '==', categoryId));
      }

      // Add region filter if not 'all'
      if (region !== 'all') {
        trendingQuery = query(trendingQuery, where('region', '==', region));
      }

      // Add limit
      trendingQuery = query(trendingQuery, firestoreLimit(limitCount));
        ...
        contentIds.map(async (contentId) => {
          ...
          if (contentDoc.exists()) {
            return {
              ...
              ...contentDoc.data(),
              trendingScore:
                trendingSnapshot.docs.find((doc) => doc.data().contentId === contentId)?.data()
                  .trendingScore || 0,
            };
          }
          return null;
  ...
  const loadMore = useCallback(async () => {
    ...
    try {
      ...
      // Add category filter if provided
      if (categoryId) {
        nextTrendingQuery = query(nextTrendingQuery, where('categoryId', '==', categoryId));
      }

      // Add region filter if not 'all'
      if (region !== 'all') {
        nextTrendingQuery = query(nextTrendingQuery, where('region', '==', region));
      }

      // Add limit
      nextTrendingQuery = query(nextTrendingQuery, firestoreLimit(limitCount));
        ...
        nextContentIds.map(async (contentId) => {
          ...
          if (contentDoc.exists()) {
            return {
              ...
              ...contentDoc.data(),
              trendingScore:
                nextTrendingSnapshot.docs.find((doc) => doc.data().contentId === contentId)?.data()
                  .trendingScore || 0,
            };
          }
          return null;
