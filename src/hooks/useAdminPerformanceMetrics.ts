      ...
      try {
        ...
        while (attempts < maxAttempts && !claimsVerified) {
          ...
          if (tokenResult?.claims.admin || tokenResult?.claims.role === 'admin') {
            ...
            attempts++;
            console.log(
              `Admin claims not found in useAdminPerformanceMetrics, attempt ${attempts} of ${maxAttempts}`
            );
        ...
        if (!claimsVerified) {
          console.error(
            'Failed to verify admin claims after multiple attempts in useAdminPerformanceMetrics'
          );
          setLoading(false);
          return;
        }
              ...
              snapshot.docs.map(async (doc) => {
                ...
                const previousRating =
                  previousReviews.length > 0
                    ? previousReviews.reduce((sum, doc) => sum + (doc.data().rating || 0), 0) /
