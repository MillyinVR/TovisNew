...
export const updateUserClaims = onDocumentUpdated('users/{userId}', async (event) => {
  ...
  if (oldRole !== newRole || beforeData?.professionalVerificationStatus !== verificationStatus) {
    console.log(
      `Role or verification changed for user ${userId}: ${oldRole} -> ${newRole}, verification: ${verificationStatus}`
    );
...
export const updateVerificationClaims = onDocumentUpdated(
...
export const setProfessionalClaim = onCall(runtimeOpts, async (request: CallableRequest) => {
  ...
  try {
    ...
    const isProfessional =
      userData?.role === 'professional' || userData?.email === 'professional@test.com';
...
export const approveProfessionalVerification = onCall(
  runtimeOpts,
...
export const rejectProfessionalVerification = onCall(
  runtimeOpts,
  async (request: CallableRequest) => {
    if (!request.auth) {
      throw new HttpsError('unauthenticated', 'User must be authenticated');
